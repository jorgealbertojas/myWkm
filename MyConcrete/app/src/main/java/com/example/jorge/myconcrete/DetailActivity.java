...
public class DetailActivity extends AppCompatActivity implements PullRequestsAdapterOnClickHandler {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Common.isOnline(getSystemService(Context.CONNECTIVITY_SERVICE))) {
            ...
            List<PullRequest> dataPersistent = new ArrayList<>();
            dataPersistent = mDataBase.getListPullRequest(mLogin,mName);

            if (dataPersistent.size()>0) {
                mRecyclerView.setAdapter(new PullRequestsAdapter(dataPersistent));
            }else{
        ...
        public void onResponse(Call<List<PullRequest>> call, Response<List<PullRequest>> response) {
            try{
                if (response.isSuccessful()) {
                    ...
                    // Insert data Pull request
                    mDataBase.insertPullRequest(data,mName,mLogin);
